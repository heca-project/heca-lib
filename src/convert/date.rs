...
pub struct HebrewDate {
    ...
    pub(crate) month: HebrewMonth,
    pub(crate) year: HebrewYear,
}
...
use calendar::{day_of_last_rh, EPOCH, FIRST_RH};
...
impl HebrewDate {
    ...
    #[must_use]
    pub(crate) fn from_gregorian(
        date: chrono::NaiveDateTime,
    ) -> Result<HebrewDate, ConversionError> {
        if date < (*FIRST_RH + Duration::days(2 + 365)) {
            return Err(ConversionError::YearTooSmall);
        }
        let days_since_first_rh = ((date - *FIRST_RH).num_days() + 2) as u32;
